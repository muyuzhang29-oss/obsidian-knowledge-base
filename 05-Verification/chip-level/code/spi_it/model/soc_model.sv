module soc_model (
  ...
  task spi_frame_write(
    ...
    fifo_data.push_back({1'b1, 1'b0, dst_port[2:0], 3'b0}); // CMD
    fifo_data.push_back(addr[16:9]);  // ADDR byte0
    fifo_data.push_back(addr[8:1]);   // ADDR byte1
    fifo_data.push_back({addr[0], 7'b0}); // ADDR byte2
    fifo_data.push_back({1'b0, 7'b0});    // Control rd_en+rd_len
    fifo_data.push_back(len);              // data_len
